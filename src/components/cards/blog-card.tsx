  ...
  return (
    <article className="group overflow-hidden rounded-lg border border-border">
      <Link href={`/${locale}/blog/${blog.slug}`} className="block relative aspect-[16/10] w-full overflow-hidden">
        <SafeImage src={blog.coverImage} alt={blog.title} fill className="object-cover transition-transform duration-500 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 33vw" />
      </Link>
      <div className="p-5">
